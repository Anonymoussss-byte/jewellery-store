  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      timeline
        ...
        .fromTo(".intro-subtitle", { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.6 }, "-=0.25")
        .fromTo(".intro-button", { opacity: 0 }, { opacity: 1, duration: 0.5 }, "-=0.2")
  ...
  return (
    ...
    >
      ...
      {particles.map((particle) => (
        <span
          key={particle.id}
          className="intro-particle sparkle opacity-0"
          style={{
            left: `${particle.left}%`,
            top: `${particle.top}%`,
            transform: `scale(${particle.scale}) translateY(18px)`,
            transitionDelay: `${particle.delay}s`
          }}
        />
      ))}
      <div className="relative z-10 flex flex-col items-center px-6 text-center">
        <div className="intro-mark luxury-border mb-8 grid h-24 w-24 place-items-center rounded-full bg-black/50 shadow-glow opacity-0">
          <Gem className="h-12 w-12 text-gold-200 drop-shadow-[0_0_24px_rgba(249,231,167,0.55)]" strokeWidth={1.2} />
        </div>
        <p className="intro-title metallic-text font-display text-4xl uppercase tracking-[0.75em] sm:text-6xl opacity-0 translate-y-7">
          Aurelia
        </p>
        <div className="intro-line my-6 h-px w-64 origin-center bg-gold-line scale-x-0" />
        <p className="intro-subtitle max-w-md text-xs uppercase tracking-[0.45em] text-diamond-200/80 opacity-0 translate-y-3">
      ...
      <button
        type="button"
        onClick={skipIntro}
        className="intro-button absolute bottom-8 right-8 z-20 rounded-full border border-white/15 px-4 py-2 text-xs uppercase tracking-[0.24em] text-diamond-200/75 opacity-0 transition hover:border-gold-300/50 hover:text-gold-100"
      >
        Enter
      </button>
    </div>
